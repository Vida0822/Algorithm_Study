...
/** 
1. 모든 경우의 수를 다 dfs하고, 중복되는 숫자 or 순서가 역행되는 숫자가 있으면 print x  
--> 분기를 끝값 종료조건에
2. backtracking으로 이전값들과 같은 or 작은 숫자가 나오면 x  --> 분기를 재귀함수 호출부분에 
 */
public class Combination_NofM_My02 {
	...
	public static void findPermutations(int curNum) {
		...
		for (int i = 1 ; i <= n ; i++) {
			...
			// 1. 이전 값들이 현재 넣으려는 값과 같거나 크다면 
			for (int j = 0; j < selectNums.size(); j++) {
                
				if(i <= selectNums.get(j) ) {
					isMade = true ; 
					break ; 
				}			
			} // j
			// 2. backtracking (해당 숫자로 자리수 채우지 않고 다시 다음 자식 노드 검사) 
			if(isMade) 
				continue ; 
            
			// 3. 검사에 걸리지 않으면 다음 자리수 채우러 재귀함수 호출 
			selectNums.add(i);
